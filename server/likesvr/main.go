...
func Run() error {
	...
	// 注册grpc server
	pb.RegisterFavoriteServiceServer(server, &service.LikeService{}) // 注册服务
	// 注册服务健康检查
	...
	if err := consulClient.Registry(config.GetGlobalConfig().SvrConfig.Host, config.GetGlobalConfig().SvrConfig.Port,
