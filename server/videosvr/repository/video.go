...
// GetVideoListByAuthorId 获取用户自己的视频列表
func GetVideoListByAuthorId(AuthorId int64) ([]int64, error) {
	var videos []int64

	db := db.GetDB()
	err := db.Model(&Video{}).Where("author_id = ?", AuthorId).Order("id DESC").Pluck("id", &videos).Error
	if err != nil && err != gorm.ErrRecordNotFound {
		if err != nil {
			log.Errorf("GetVideoListByAuthorId err==&v", err)
		}
		return videos, err
	}
...
// GetVideoListByFeed 获取视频（比如我tiktok 下拉，获取视频）
func GetVideoListByFeed(currentTime int64) ([]Video, error) {
	...
	if err != nil && err != gorm.ErrRecordNotFound {
		log.Errorf("GetVideoListByFeed err ==%v", err)
		return videos, err
	}
...
func GetVideoInfo(vid int64) (Video, error) {
	var video Video
	Db := db.GetDB()
